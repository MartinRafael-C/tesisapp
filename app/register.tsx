...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    // Validación de Formulario Paso 2 (Simulación de Licencia)
    if (step === 2 && (cardNumber.length < 19 || expiry.length < 5 || cvv.length < 3)) {
      Alert.alert("SISTEMA", "COMPLETE LOS DATOS DE LA LICENCIA"); 
      return; 
    }
    ...
    if (step === 1) { 
        if(!email || !password || !fullName) {
            Alert.alert("SISTEMA", "TODOS LOS CAMPOS SON REQUERIDOS");
            return;
        }
        setStep(2); 
        return; 
    ...
    if (error) { 
      Alert.alert("ERROR_AUTH", error.message); 
      setLoading(false); 
      return; 
    }
...
const styles = StyleSheet.create({
  ...
  stepIndicator: { color: THEME.teal, fontSize: 10, letterSpacing: 2, marginBottom: 8 },
  title: { fontSize: 32, color: THEME.white, letterSpacing: -1, fontWeight: '900' },
  form: { gap: 20 },
  inputBox: { gap: 8 },
  label: { color: THEME.teal, fontSize: 10, fontWeight: 'bold', marginLeft: 4 },
  input: { 
  ...
  btnPrimary: { 
    backgroundColor: THEME.teal, padding: 20, borderRadius: 2, flexDirection: 'row',
    justifyContent: 'center', alignItems: 'center', gap: 10, marginTop: 10 
  },
  btnPay: { 
    backgroundColor: '#1B5E20', padding: 20, borderRadius: 2, flexDirection: 'row',
    justifyContent: 'center', alignItems: 'center', gap: 10
  },
  btnText: { color: THEME.white, fontWeight: 'bold', fontSize: 14, letterSpacing: 1 },
  cardBox: { 
    backgroundColor: THEME.petroleo, padding: 24, borderRadius: 2, borderWidth: 1, 
    borderColor: THEME.teal, gap: 15 
  },
  priceRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 },
  cardHeader: { fontWeight: 'bold', color: THEME.teal, fontSize: 10, letterSpacing: 1 },
  price: { color: THEME.white, fontSize: 18, fontWeight: '900' },
  cardInput: { 
    borderBottomWidth: 1, borderBottomColor: THEME.glass, color: THEME.white, 
    paddingVertical: 10, fontFamily: 'monospace', fontSize: 16
  },
  backBtn: { marginTop: 20, padding: 10, alignSelf: 'center' },
  backText: { color: 'rgba(255,255,255,0.4)', fontSize: 11, letterSpacing: 1 }
});